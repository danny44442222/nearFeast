//
//  ReviewReplyViewController.swift
//  nearFeast
//
...
class ReviewReplyViewController: UIViewController {
    ...
    func loadData(){
        ...
        FirebaseData.getDishReviewWithCommentList(self.dishData.docId, reviewid: self.reviewData.docId) { error, userData in
            self.stopAnimating()
            if let error = error{
    ...
    @IBAction func sendBtnPressed(_ sender:Any){
        ...
        FirebaseData.saveProdReviewCommData(dishid: self.dishData.docId, reviewid: self.reviewData.docId, uid: uuid, userData: coment) { error in
            self.stopAnimating()
            if let error = error{
                PopupHelper.showAlertControllerWithError(forErrorMessage: error.localizedDescription, forViewController: self)
            }
...
extension ReviewReplyViewController:UITableViewDelegate,UITableViewDataSource{
