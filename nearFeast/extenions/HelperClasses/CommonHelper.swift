...
class CommonHelper
{
    ...
    class func getCachedUserData() -> userLogin?{
        
        let defaults = UserDefaults.standard
        if let studentData = defaults.object(forKey: "saveUser") as? Data {
            let decoder = JSONDecoder()
            if let student = try? decoder.decode(userLogin.self, from: studentData) {
               return student
            }
            else{
                return nil
...
//
    ...
    class func saveCachedUserData(userData:userLogin){
